...
def flagcal_bl(msfile, params, niters=1, flagger='default', interactive=False):
    ...
    while flagcal_fin == 'n':
        ...
        cts.blcal(msfile, 
                  ...
                  gaintable=params['calibration']['gaintables'], # all prior cal
...
def imagecal(targetcalfile, params, nloops=1, ploops=5, aploops=1, flagger='default', interactive=False):
    ...
    for nindex in nloop_index:
        ...
        sc_p_msfile = targetcalfile # start again with the avspcfile ms file name for the next round of self-cals
        for pindex in ploop_index: # Run all the ploops
            print('Self-cal phase-only loop', pindex)
            imgr.tcleaner(sc_p_msfile, params, threshold=threshold_range[pindex - 1], niter=niter_range[pindex - 1], outimage=tempdir+outimage+'_sc_p.'+str(nindex)+str(pindex), interactive=interactive) 
            # Adding imstats in the list
            imagename = tempdir+outimage+'_sc_p.'+str(nindex)+str(pindex)+'.image.tt0'
            type_list.append('p')
            ...
            clb.selfcal(sc_p_msfile, params, mode='p', in_gaintable=[], out_gaintable=tempdir+'sc_p.gcal.'+str(nindex)+str(pindex), solint=solint_range[pindex - 1], solnorm = False)
            gaintable = [tempdir+'sc_p.gcal.'+str(nindex)+str(pindex)] # Change the gaintable to the latest
            
            cts.applycal(sc_p_msfile, gaintable=gaintable, field='', gainfield='', applymode='calonly', interp=['linear'], calwt=False, parang=False)
            cts.mstransform(sc_p_msfile, field='0', spw='0', datacolumn='corrected', outputvis=tempdir+'sc_p_'+str(nindex)+str(pindex)+'.ms')
        ...
        for apindex in aploop_index: # Run all the aploops
            ...
            imgr.tcleaner(sc_ap_msfile, params, threshold=threshold_ap, niter=niter_ap, outimage=tempdir+outimage+'_sc_ap.'+str(nindex)+str(apindex), interactive=interactive)
            
            # Adding the self cal ap loop imstats in the list
            imagename = tempdir+outimage+'_sc_ap.'+str(nindex)+str(apindex)+'.image.tt0'
            type_list.append('ap')
            loop_list.append(nindex)
            ...
            clb.selfcal(sc_ap_msfile, params, mode='a&p', in_gaintable=[], out_gaintable=tempdir+'sc_ap.gcal.'+str(nindex)+str(apindex), solint=solint_ap, solnorm=True)
            gaintable = [tempdir+'sc_ap.gcal.'+str(nindex)+str(apindex)] # Change the gaintable to the latest
            cts.applycal(sc_ap_msfile, gaintable=gaintable, field='', gainfield='', applymode='calonly', interp=['linear'], calwt=False, parang=False)
            cts.mstransform(sc_ap_msfile, field='0', spw='0', datacolumn='corrected', outputvis=tempdir+'sc_ap_'+str(nindex)+str(apindex)+'.ms')
        ...
        imagename = tempdir+outimage+'_sc_ap_final.'+str(nindex)+str(apindex)+'.image.tt0'
        ...
        cts.applycal(targetcalfile, gaintable=gaintable, field='', gainfield='', applymode='calonly', interp=['linear'], calwt=False, parang=False) # Apply the latest calibration table from the last loop
        if nterms == 2:
            cts.ft(targetcalfile, spw='0', nterms=nterms, model=[tempdir+outimage+'_sc_ap_final.'+str(nindex)+str(apindex)+'.model.tt0', tempdir+outimage+'_sc_ap_final.'+str(nindex)+str(apindex)+'.model.tt1'], usescratch=True) # Copy the finale model from the tempfile
        elif nterms == 1:
            cts.ft(targetcalfile, spw='0', nterms=nterms, model=[tempdir+outimage+'_sc_ap_final.'+str(nindex)+str(apindex)+'.model.tt0'], usescratch=True) # Copy the finale model from the tempfile

        cts.uvsub(targetcalfile, reverse=False)
        flg.rflagger(targetcalfile, params, field=target, tcut=6, fcut=6, instance='postcal')
        flg.extend(targetcalfile, params, field=target, grow=80, instance='postcal')
        cts.uvsub(targetcalfile, reverse=True) # Adding the corrected-data and residuals
    ...
    t = Table((np.array(loop_list), np.array(type_list), np.array(index_list), np.array(rms_list), np.array(peakflux_list)), names=('loop', 'type', 'index', 'rms', 'peakflux'))
    ...
    for i in nloop_index:
        ...
        for k,j in enumerate(['p', 'ap', 'final']):
            ...
            plt.plot(sub_subtable['index']+index_add, dyna_range, marker=markers[k])
    ...
    subprocess.run('cp -r {} {}'.format(gaintable[0], outdir_ap_gaintable[0]), shell=True, check=True) #cp the final gaintable to the outdir 
    subprocess.run('cp -r {} {}'.format(final_pcal_table[0], outdir_p_gaintable[0]), shell=True, check=True) #cp the final gaintable to the outdir 
...
def imagecal2(targetcalfile, params, nloops=1, ploops=5, aploops=1, flagger='default', interactive=False):
    ...
    for nindex in nloop_index:
        #subprocess.run('rm -r {}'.format(temp_dir), shell=True, check=True) # Clearing the temp directory
        #subprocess.run('mkdir {}'.format(temp_dir), shell=True, check=True) 
        ploop_index = 1 + np.arange(ploops) #This gives the index to the cal files created later
        
        for pindex in ploop_index: # Run all the ploops
            print('Self-cal phase-only loop', pindex)
            imgr.tcleaner(targetcalfile, params, threshold=threshold_range[pindex - 1], niter=niter_range[pindex - 1], outimage=tempdir+outimage+'_sc_p.'+str(nindex)+str(pindex), interactive=interactive) 
            # Adding imstats in the list
            imagename = tempdir+outimage+'_sc_p.'+str(nindex)+str(pindex)+'.image.tt0'
            type_list.append('p')
            loop_list.append(nindex)
            ...
            clb.selfcal(targetcalfile, params, mode='p', in_gaintable=[], out_gaintable=tempdir+'sc_p.gcal.'+str(nindex)+str(pindex), solint=solint_range[pindex - 1], solnorm = False)
            gaintable = [tempdir+'sc_p.gcal.'+str(nindex)+str(pindex)] # Change the gaintable to the latest 
            cts.applycal(targetcalfile, gaintable=gaintable, field='', gainfield='', applymode='calonly', interp=['linear'], calwt=False, parang=False)
        ...
        for apindex in aploop_index: # Run all the aploops
            print('Self-cal a&p loop', apindex)
            imgr.tcleaner(targetcalfile, params, threshold=threshold_ap, niter=niter_ap, outimage=tempdir+outimage+'_sc_ap.'+str(nindex)+str(apindex), interactive=interactive)
            ...
            clb.selfcal(targetcalfile, params, mode='a&p', in_gaintable=[], out_gaintable=tempdir+'sc_ap.gcal.'+str(nindex)+str(apindex), solint=solint_ap, solnorm=True)
            gaintable = final_pcal_table + [tempdir+'sc_ap.gcal.'+str(nindex)+str(apindex)] # Change the gaintable to the latest, this will also have the phaseonly calibration solutions applied.
            cts.applycal(targetcalfile, gaintable=gaintable, field='', gainfield='', applymode='calonly', interp=['linear'], calwt=False, parang=False)
