...
def bpasscal2(msfile, params, field=None):
    ...
    for i in range(ini_fgloops):
        print('Running the single channel flagcal loop pre-bandpass ...')

        cts.gaincal(msfile, caltable=bpassgaintable, field=field, uvrange=uvran, spw=ch0_spw, solint = '120s', solmode='R', refant = refant, minsnr = 2.0, gaintype = 'G', calmode = 'ap', gaintable = gaintables, interp = ['linear, linear',], append=True, parang = False)
         
        for j in fluxcal:
            #print('Applying the calibration table to ', i)
            cts.applycal(msfile, field=i, spw=ch0_spw, gaintable=bpassgaintable, gainfield=[
